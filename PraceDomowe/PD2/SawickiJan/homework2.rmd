...
First we take a look at the results for all three methods and their results for small dataset. 
...
The problem is that is uses a function from a library and has to load this very library to work. 

```{}
library(rlang)
```

Now, one may claim that it would be more more accurate to measure the function WITHOUT loading the library, but it is clearly unfair. 
...
It appears that the time of computations of the function grows faster than the time needed to apply out NSE magic.
