...
get_argument_name <- function(original){
  ...
  argslist_name <- "argslist"
  while(argslist_name %in% names(get_formals(original))) 
    argslist_name <- paste0(argslist_name, "1")
  argslist_name
}

fix_argument_name <- function(new_name, FUNCTION){
  # Funkcja zastępuje nazwę pierwszego argumentu funkcji FUNCTION (argslist) nazwą new_name.
  # Zakładamy, że w ciele funkcji jedyne użycia argslist odwołują się do jej pierwszego argumentu.
  # Występuje w wersji 1 i 2.
  new_body <- capture.output(body(FUNCTION)) %>%
    stri_replace_all_fixed("argslist", new_name) %>%
    str2expression()
...
get_formals <- function(FUNCTION){
  ...
  # Nie interesują nas dokładne wartości liczb powyżej 10 - możemy je zmapować na 9. 
  ...
  # brackets_as_int niesie informacje o nawiasach wewnątrz listy argumentów funkcji.
  # inner_brackets - znajduje wszystkie pozycje startu i końca wywołań funkcji wewnątrz listy argumentów.
...
Pomysł jest prosty - zwracana funkcja przyjmuje 2 argumenty: `argslist` - listę, data.frame lub coś środowiskopodobnego (domyślnie: środowisko wzywającego funkcję `FUNCTION`) i elipsę.

Oczywiście, może się zdarzyć, że funkcja `FUNCTION` wśród swoich argumentów ma argument o nazwie `argslist`. Wówczas przy pomocy funkcji pomocniczych `get_argument_name()` i `fix_argument_name()` następuje poprawka. Tym niemniej, takie sytuacje należą do rzadkości - dla ustalenia uwagi w dalszej części pracy zakładam, że pierwszy argument *rzeczywiście* nazywa się `argslist`.

```{r version_1}
NSE_factory_1 <- function(FUNCTION){
  stopifnot(is_function(FUNCTION))
  out <- function(argslist = caller_env(), ...){
    given_args <- quos(...)
    evaluated_args <- lapply(given_args, function(expr) eval(quo_squash(expr), argslist))
    do.call(FUNCTION, evaluated_args)
  }
  correct_argument_name <- get_argument_name(FUNCTION) 
  if(correct_argument_name != "argslist")
...
Pomocna była funkcja `rlang::fn_fmls()`. Niestety, nie działa na funkcje prymitywne - tu musiałem radzić sobie po swojemu. 

Wersja druga funkcji zwraca funkcję, która na pierwszym miejscu ma argument `argslist` - jak w wersji pierwszej. Pozostałe argumenty są identyczne, jak funkcji FUNCTION.

```{r version_2}
NSE_factory_2 <- function(FUNCTION){
  stopifnot(is_function(FUNCTION))
  arguments <- get_formals(FUNCTION)
  # Dołączamy do listy argumentów argslist z wartością domyślną "caller_env()".
  arguments <- append(arguments, call("caller_env"), 0)
  names(arguments)[[1]] <- "argslist"
  out <- function() {
    ...
    # Ignorujemy argumenty argslist i elipsę.
    for(name in arguments_names[arguments_names != "..."]){
      # Sytuacja wygląda tak: name <- arg1 <- val1. Jeśli np ktoś wywołał funkcję f(argslist, x = a), 
      # to name jest referencją na x, x jest referencją na obiekt a, 
      # który trzeba zewaluować w środowisku argslist.
      ...
      new_value <- eval(eval(new_name) ,argslist)
    ...
    if("..." %in% arguments_names){
      ...
      arguments_extra_evaluated <- lapply(arguments_extra, 
             function(expr) eval(quo_squash(expr), argslist))
  ...
  correct_argument_name <- get_argument_name(FUNCTION) 
  if(correct_argument_name != "argslist") out <- fix_argument_name(correct_argument_name, out)
  out
}
...
NSE_factory_3 <- function(FUNCTION){
  ...
  out <- function() {
    # tu się znajdzie:
    # if(!is_missing(arg1))
    #   arg1 <- eval(substitute(arg1), argslist)
    # if(!is_missing(arg2))
    # arg2 <- eval(substitute(arg2), argslist)
    # ...
    ...
    # argumentów w środowisku argslist. 
    ...
    #          function(expr) eval(quo_squash(expr), argslist)) -> arguments_extra_evaluated
...
results_df <- do.call(rbind, results) %>% 
  mutate(Function = rep(c("min", "mean", "lm", "unlist", "other_unlist"), 
                                    each = 75)) 
bwplot(log10(time) ~ expr | Function , results_df, title = "Czasy generowania funkcji")
```
 ...
 * W większości funkcji widzimy pewną prawdłowość. Ogółem rzecz ujmując, poza oryginałem najlepiej radzą sobie wyniki wersji pierwszej, zaraz za nimi wyniki wersji trzeciej, a wersja druga zostaje daleko w tyle. Prawdopodobnie jest to wina pętli `for()`.  
