...
### Porównanie szybkości generowania wykresów [ns]
...
first_plot_final_data <- plot_final_data %>% 
  filter(plot != "point_plot" & plot != "bar_plot")

show_summary_bar_chart(first_plot_final_data)
```

Na powyższym wykresie można zauważyć, że we wszystkich trzech przypadkach (generowanie wykresu pudełkowego, histogramu oraz wykresu gęstości) najszybszy był pakiet `ggplot`. Jednakże, na następnym wykresie wyraźnie widać, że istnieje przypadek, gdy biblioteka `ggplot` nie jest najlepsza (pod względem szybkości).
...
Dzieje się tak w przypadku generowania wykresu słupkowego. Warto również zwrócić uwagę na znaczącą różnicę pomiędzy pakietem `ggplot` a pozostałymi w szybkości generowania wykresu punktowego.